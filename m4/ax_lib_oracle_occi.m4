...
#   Copyright (c) 2018 Anton Shapka <66ton99@gmail.com>
#
...
--with-occi-include and --with-occi-lib options.])
    ...
    if test -n "$oracle_include_dir" -a -n "$oracle_lib_dir"; then
        ...
        if test -n "$oracle_include_dir2"; then
            ...
            oracle_include_dir="$oracle_include_dir2"
        fi
        ...
        dnl
        AC_MSG_CHECKING([for Oracle OCCI headers 10.x of 11.x in $oracle_include_dir])
...
env = oracle::occi::Environment::createEnvironment();
    ...
    dnl
    dnl Check OCCI files >=12.0
    dnl
    if test -n "$oracle_include_dir3" -a -n "$oracle_lib_dir"; then
        ...
        dnl Check OCCI headers
        dnl
        AC_MSG_CHECKING([for Oracle OCCI headers 12.2 in $oracle_include_dir])
        ...
        AC_COMPILE_IFELSE([
            AC_LANG_PROGRAM([[@%:@include <occi.h>]],
                [[
#if defined(OCCI_MAJOR_VERSION)
#if OCCI_MAJOR_VERSION >= 12
// Oracle >= 12.0 detected
#else
#  error Oracle occi.h header not found
#endif
#endif
    ...
    if test "$occi_header_found" = "yes" -a "$occi_lib_found" = "yes" -a \
        ...
        if test -n "$oracle_version_major" -a -n $"oracle_version_minor"; then
            ...
            if test "$oracle_version_check" = "1"; then
                ...
                AC_MSG_CHECKING([for Oracle version >= 11.x and < 12.x to use -lnnz11 flag])
                oracle_nnz11_check=`expr \( $oracle_version_number \>\= 11 \* 1000000 \) \& \( $oracle_version_number \< 12 \* 1000000 \)`
    ...
    AC_MSG_CHECKING([if Oracle $ORACLE_OCCI_VERSION support is enabled])
